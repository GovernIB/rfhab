...
public class PrincipalController {
	...
	public ModelAndView canviarIdioma(HttpServletRequest request, HttpServletResponse response,
		...
		es.caib.rfhab.back.utils.RFHabSessionLocaleResolver.setLocaleManually(request, idioma);		
		return new ModelAndView("principal");
	}
	...
	public ModelAndView canviarPipella(HttpServletRequest request, HttpServletResponse response,
		...
		if (pipella != null && pipella.trim().length() != 0) {
			...
			if ("user".equals(pipella)) {
				loginInfo.setRolActual("usuari");
				return new ModelAndView(new RedirectView("/usuari/home", true));
			}
			...
			if ("super".equals(pipella)) {
				loginInfo.setRolActual("superadmin");
				return new ModelAndView(new RedirectView("/superadmin/entitat/list", true));
			}

			if ("webdb".equals(pipella)) {
				loginInfo.setRolActual("webdb");
