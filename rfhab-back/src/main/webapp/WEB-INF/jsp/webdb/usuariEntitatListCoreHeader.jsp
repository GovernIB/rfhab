...
<un:useConstants var="UsuariEntitatFields" className="es.caib.rfhab.model.fields.UsuariEntitatFields"/>
        ...
        <th>${rfh:getSortIcons(__theFilterForm,UsuariEntitatFields.USUARIENTITATID)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,UsuariEntitatFields.ENTITATID)}">
        <th>${rfh:getSortIcons(__theFilterForm,UsuariEntitatFields.ENTITATID)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,UsuariEntitatFields.USUARIID)}">
        <th>${rfh:getSortIcons(__theFilterForm,UsuariEntitatFields.USUARIID)}</th>
        </c:if>
        <c:if test="${!gen:contains(__theFilterForm.hiddenFields,UsuariEntitatFields.ACTIU)}">
        <th>${rfh:getSortIcons(__theFilterForm,UsuariEntitatFields.ACTIU)}</th>
        </c:if>
