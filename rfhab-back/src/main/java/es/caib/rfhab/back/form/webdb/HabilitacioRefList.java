 ...
 */
@Component
public class HabilitacioRefList extends RefListBase implements HabilitacioFields {

    @EJB(mappedName = HabilitacioService.JNDI_NAME)
    private HabilitacioService habilitacioEjb;

    @EJB(mappedName = TraduccioService.JNDI_NAME)
    private TraduccioService traduccioEjb;
    public HabilitacioRefList(HabilitacioRefList __clone) {
        super(__clone);
        this.habilitacioEjb = __clone.habilitacioEjb;
        this.traduccioEjb = __clone.traduccioEjb;
    }

    public HabilitacioRefList() {
    ...
    public List<StringKeyValue> getReferenceList(Field<?> keyField, Where where, OrderBy ... orderBy) throws I18NException {
        ...
        List<StringKeyValue> list = habilitacioEjb.executeQuery(select, where, (orderBy==null || orderBy.length == 0) ? getOrderBy() : orderBy);
        if (_transSelect == null) {
            return list;
        }
        // key => TransID | value => habilitacioEjb_PK
        ...
        _w1 = es.caib.rfhab.model.fields.TraduccioFields.TRADUCCIOID.in(habilitacioEjb.getSubQuery(_transSelect, where));
