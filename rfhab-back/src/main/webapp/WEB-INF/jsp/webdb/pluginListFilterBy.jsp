...
<un:useConstants var="PluginFields" className="es.caib.rfhab.model.fields.PluginFields"/>
  ...
  <div id="FilterDiv" class="wellgroupfilter formbox" style="${displayFilterDiv} margin-bottom:3px; margin-left: 1px; padding:3px;">
      ...
      <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PluginFields.NOM)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="plugin.nom" var="nom" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PluginFields.DESCRIPCIO)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="plugin.descripcio" var="descripcio" />
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PluginFields.ENTITATID)}">
            <div class="input-group" style="padding-right: 4px;padding-bottom: 4px;">
            <%-- FILTRE NUMERO DESDE-FINS --%>
              <span class="add-on"><fmt:message key="plugin.entitatID" />:</span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PluginFields.PROPERTIES)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="plugin.properties" var="properties" />
              <fmt:message key="genapp.form.searchby" var="cercaperproperties" >                
                 <fmt:param value="${properties}"/>
              </fmt:message>
              <span class="add-on"><c:out value="${properties}" />:</span>
              <form:input cssClass="search-query input-medium" placeholder="${cercaperproperties}" path="properties" />
            </div>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PluginFields.DATACREACIO)}">
<%-- FILTRE DATE-TIME --%>
            <div class="input-group" style="padding-right:4px;padding-bottom:4px;align-items:center;">
              <span class="add-on"><fmt:message key="plugin.dataCreacio" />:</span>
              <span class="add-on">&nbsp;<fmt:message key="genapp.from" /></span>
        ...
        <c:if test="${gen:contains(__theFilterForm.filterByFields ,PluginFields.TIPUS)}">
            <%-- FILTRE STRING --%>
            <div class="input-prepend" style="padding-right: 4px;padding-bottom: 4px;">
              <fmt:message key="plugin.tipus" var="tipus" />
