...
@Table(name = "rfh_fitxer" , indexes = { 
        @Index(name="rfh_fitxer_pk_i", columnList = "fitxerid")})
@SequenceGenerator(name="FITXER_SEQ", sequenceName="rfh_fitxer_seq", allocationSize=1, initialValue=1000)
...
// EXP  Field:fitxerid | Table: rfh_digitalib | Type: 0  
  ...
  void postLoad() {
    if (threadLocal.get() != null) {
      this.encryptedFileID = es.caib.rfhab.hibernate.HibernateFileUtil.encryptFileID(fitxerID);
    }
  }
 ...
 // ---------------  STATIC METHODS ------------------
  ...
  static FitxerJPA copyJPA(FitxerJPA __jpa,
    ...
    if(!"ScanWebJPA".equals(origenJPA) 
      ...
      __tmp.setScanWebs(ScanWebJPA.copyJPA(__jpa.getScanWebs(), __alreadyCopied,"FitxerJPA"));
    }
