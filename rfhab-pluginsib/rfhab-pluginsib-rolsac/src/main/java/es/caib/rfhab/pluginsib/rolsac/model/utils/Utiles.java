package es.caib.rfhab.pluginsib.rolsac.model.utils;
...
import es.caib.rfhab.pluginsib.rolsac.model.ibitrolsac.Traduccion;
...
public class Utiles {
    ...
    // private static String[] GETTERS_INVALIDS =
    // {"getSerializer","getDeserializer", "getTypeDesc"};
    // private static String[] SETTERS_INVALIDS = {"setSerializer",
    // "setDeserializer", "setTypeDesc"};
     ...
     * propiedad del mismo nombre en entity.
     * con excepción de idioma, que normalmente no está en el entity y se usa el que
    ...
    public static void copyProperty(Object entity, Object dto, Method setter, String lang) throws SecurityException,
        ...
        // si estamos intentando obtener el idioma, como no está en el dto usamos el que
        // nos llega
        // del request.(en algunos casos si que aparece pero deberia venir a nulo, si no
        // es asi
        ...
        if ("idioma".equals(property)) {
             ...
             * tienen booleanos (no hay getters
             * que empiecen por isXxx). Nota 2: Se busca primero en la traduccion porque en
             * Documento y DocumentTramit
             * existe una propiedad con el mismo nombre en la clase y en la traduccion. La
             * propiedad de la clase ya no se
        ...
        if (value != null) {
             ...
             * propiedad de entity. Se usa el
             * metodo menos restrictivo de BeanUtils en vez del propio de Java debido a un
             * bug relacionado con tipos
