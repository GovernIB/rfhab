...
import es.caib.rfhab.back.validator.webdb.EntitatWebValidator;
 ...
 * @author GenApp
 */
@MenuOption(labelCode="entitat.entitat.plural", order=20, group=Tab.MENU_WEBDB)
@Controller
...
public class EntitatController
    extends es.caib.rfhab.back.controller.RFHabBaseController<Entitat, java.lang.Long> implements EntitatFields {

  @EJB(mappedName = es.caib.rfhab.ejb.EntitatService.JNDI_NAME)
  ...
  public void fillReferencesForForm(EntitatForm entitatForm,
    ...
    if (entitatForm.getListOfUnitatForUnitatID() == null) {
      List<StringKeyValue> _listSKV = getReferenceListForUnitatID(request, mav, entitatForm, null);

      if(_listSKV != null && !_listSKV.isEmpty()) { 
          java.util.Collections.sort(_listSKV, STRINGKEYVALUE_COMPARATOR);
      }
      entitatForm.setListOfUnitatForUnitatID(_listSKV);
    }
...
public java.lang.Long stringToPK(String value) {
 ...
 exceptionMsg };
  ...
  public List<StringKeyValue> getReferenceListForUnitatID(HttpServletRequest request,
    ...
    if (entitatForm.isHiddenField(UNITATID)) {
      return EMPTY_STRINGKEYVALUE_LIST;
    }
    Where _where = null;
    if (entitatForm.isReadOnlyField(UNITATID)) {
      _where = UnitatFields.UNITATID.equal(entitatForm.getEntitat().getUnitatID());
    }
  ...
  public List<StringKeyValue> getReferenceListForUnitatID(HttpServletRequest request,
    ...
    if (entitatFilterForm.isHiddenField(UNITATID)
       && !entitatFilterForm.isGroupByField(UNITATID)) {
      return EMPTY_STRINGKEYVALUE_LIST;
    }
    Where _w = null;
    if (!_groupByItemsMap.containsKey(UNITATID)) {
      ...
      for (Entitat _item : list) {
        if(_item.getUnitatID() == null) { continue; };
