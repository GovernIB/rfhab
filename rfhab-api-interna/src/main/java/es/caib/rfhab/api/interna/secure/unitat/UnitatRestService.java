package es.caib.rfhab.api.interna.secure.unitat;
...
@SecurityScheme(type = SecuritySchemeType.HTTP, name = UnitatRestService.SECURITY_NAME, scheme = "basic")
public class UnitatRestService extends RestUtils {

    protected Logger log = Logger.getLogger(UnitatRestService.class);
    ...
    public static class ConsultaUnitatResponse {
    ...
    @SecurityRequirement(name = UnitatRestService.SECURITY_NAME)
    @Produces(MediaType.APPLICATION_JSON)
    @Consumes(MediaType.APPLICATION_JSON)
    public String consultaPerCodiDir3(
            ...
            @Parameter(description = "Codi DIR3", required = true) @QueryParam("codidir3") @NotNull String codiDir3) {
        try {
            if (language == null || language.isEmpty()) {
                language = "ca";
            }
            ConsultaUnitatResponse response = new ConsultaUnitatResponse();
            Unitat unitat = unitatLogicaEjb.findByCodiDir3(codiDir3);
            if (unitat != null) {
                response.setExisteix(true);
                response.setUnitatId(unitat.getUnitatID());
            } else {
                response.setExisteix(false);
            }
            return mapper.writeValueAsString(response);
        } catch (Throwable th) {
            String msg = I18NLogicUtilsApiInterna.tradueix(new Locale(language), "unitat.error.desconegut",
